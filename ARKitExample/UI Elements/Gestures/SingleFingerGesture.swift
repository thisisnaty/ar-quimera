...
class SingleFingerGesture: Gesture {
    ...
    func finishGesture() {
        ...
        //Rota el objeto, ufo, 1 radian con respeto eje Y durante 1 segundo, 20 veces.
            object?.runAction(SCNAction.repeat(SCNAction.rotateBy(x: 0, y: 1, z: 0, duration: 1), count: 20))
            }
            
        //Mover objeto 2 metros en direccion eje Y en 7 segundos (elevacion)
            object?.runAction(SCNAction.move(by: SCNVector3(x: 0, y: 0.7, z: 0), duration: 7))
